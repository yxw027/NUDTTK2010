...
//生成者：杨诚鋆
//与Leo仿真区别：
//1.加入旁瓣信号范围（约40°，来源：The GPS Block IIR/IIR-M Antenna Panel Pattern Appendix A – SV-Specific Patterns, Plots.）
//2.天线方向朝向天底
//////////////////////////////////////////////////////////////////
...
namespace NUDTTK
{
   ...
   //               若 distance2<30,000km     则判定为MEO卫星 覆盖角为： 25°
		...
		bool GPSTQRecObsSimu::simuGPSObsFile(Rinex2_1_ObsFile &obsFile, char cSatSystem)
		{
			...
			if(cSatSystem == 'C')// 2021.09.22杨诚鋆修改  FREQUENCE_L2 = BD_FREQUENCE_L5→FREQUENCE_L2 = BD_FREQUENCE_L2
			{
			...
			else if(cSatSystem == 'C')//2021.09.07杨诚鋆修改 TYPE_OBS_P5→TYPE_OBS_P2     TYPE_OBS_L5→TYPE_OBS_L2
			{
				obsFile.m_header.pbyObsTypeList[1] = TYPE_OBS_P2;
				obsFile.m_header.pbyObsTypeList[3] = TYPE_OBS_L2; // 20141126, 与频率相关
			}			
