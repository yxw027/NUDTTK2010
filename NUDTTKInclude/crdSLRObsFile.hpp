...
namespace NUDTTK
{
	namespace SLR
	{
		struct CRDHeaderRecord
		{
			...
			CRDHeaderRecord()
			{
				memset(this,0,sizeof(CRDHeaderRecord));				
			}
		};
		struct CRDConfigRecord
		{
			...
			CRDConfigRecord()
			{
				memset(this,0,sizeof(CRDConfigRecord));				
			}
		};
		struct CRDDataRecord                 
		{
			...
			double   NPTWindowLength;        // F6.1,s
			int      nRawRangesNum;          // I6
			double   BinRMS;                 // F9.1,ps
			double   BinSkew;                // F7.3
			double   BinKurtosis;            // F7.3
			double   BinPeak_Mean;           // F9.1
			double   ReturnRate;             // F5.1
			int      nDetectorChannel;       // I1
			...
			CRDDataRecord()
			{
				//通常20和11作为一组数据，但当气象数据变化不大时，20数据仅出现一次
				memset(this,0,sizeof(CRDDataRecord));				
			}
		};
		struct CRD405060Record
		{
			...
			CRD405060Record()
			{				
				memset(this,0,sizeof(CRD405060Record));				
			}
		};
		struct crdSinglePassArc
		{
			CRDHeaderRecord         crdHeader;
			CRDConfigRecord         crdConfig;
			vector<CRDDataRecord>   crdDataRecordList;
			CRD405060Record         crd405060;
			UTC getTime(CRDDataRecord Record);
		};
		...
		public:
			~crdSLRObsFile(void);
		public:			
			bool open(string strCRDFileName);	
			bool write(string strCRDFileName);
		public:			
			vector<crdSinglePassArc>     m_data;
		};
	}
}