...
namespace NUDTTK
{
	...
	//
	// 参数：
	// 1 string& sDelim - 数字之间的分隔符，默认为空格
	// 2 bool bLineBreak - 行与行之间是否有回车换行符，默认为真(有换行符)
	//
	// 返回值：string 型，转换得到的字符串
	//////////////////////////////////////////////////////////////////////
	string Matrix::ToString(string sDelim /*= " "*/, bool bLineBreak /*= TRUE*/)
	{
		string s="";
		for (int i=0; i<m_nNumRows; i++)
		{
			...
			{
				char ss[1000]; // 将 ss 的大小调大一些，防止溢出 20070917
				sprintf(ss,"%15.4le", GetElement(i, j));
				s += ss;
	...
	// 2. const string& sDelim - 数字之间的分隔符，默认为空格
	//
	// 返回值：string 型，转换得到的字符串
	//////////////////////////////////////////////////////////////////////
	string Matrix::RowToString(int nRow, string sDelim /*= " "*/)
	{
		string s = "";
		...
		{
			char ss[100];
			sprintf(ss,"%20f", GetElement(nRow, j));
			s += ss;
			if (j != m_nNumColumns-1)
				s += sDelim;
		}
	...
	// 返回值：string 型，转换得到的字符串
	//////////////////////////////////////////////////////////////////////
	string Matrix::ColToString(int nCol, string sDelim /*= " "*/)
	{
		string s = "";
		...
		{
			char ss[100];
			sprintf(ss,"%20f", GetElement(i, nCol));
			s += ss;
			if (i != m_nNumRows-1)
				s += sDelim;
		}
	...
	//
	// 参数：
	// 1. const Matrix& AR - 左边复矩阵的实部矩阵
	// 2. const Matrix& AI - 左边复矩阵的虚部矩阵
	// 3. const Matrix& BR - 右边复矩阵的实部矩阵
	// 4. const Matrix& BI - 右边复矩阵的虚部矩阵
	// 5. Matrix& CR - 乘积复矩阵的实部矩阵
	// 6. Matrix& CI - 乘积复矩阵的虚部矩阵
	//
	// 返回值：BOOL型，复矩阵乘法是否成功
	//////////////////////////////////////////////////////////////////////
	BOOL Matrix::CMul(const Matrix& AR, const Matrix& AI, const Matrix& BR, const Matrix& BI, Matrix& CR, Matrix& CI) const
	{
		// 首先检查行列数是否符合要求
		...
		// 构造乘积矩阵实部矩阵和虚部矩阵
		Matrix mtxCR(AR.GetNumRows(), BR.GetNumColumns()), mtxCI(AR.GetNumRows(), BR.GetNumColumns());
		// 复矩阵相乘
	...
	Matrix Matrix::Inv() const
	{
		// 构造目标矩阵 
		Matrix	Inv=*this;
		Inv.InvertGaussJordan();//利用全选主元高斯－约当法
		return Inv;
	}
	...
	// 返回值：Matrix型，指定矩阵逆矩阵
	//////////////////////////////////////////////////////////////////////
	Matrix Matrix::Inv_Ssgj() const
	{
		// 构造目标矩阵 
		Matrix	Inv = *this;
		Inv.InvertSsgj();// 利用全选主元高斯－约当法
		return Inv;
	}

	//////////////////////////////////////////////////////////////////////
	// 三对角矩阵的求逆--求逆的同时保持原矩阵不变
	//
	// 参数：无
	//
	// 返回值：Matrix型，指定矩阵逆矩阵
	//////////////////////////////////////////////////////////////////////
	Matrix Matrix::InvertTriAngle() const
	{
		if(m_nNumRows <= 2)	
		{
			return this->Inv();
		}
		Matrix A(m_nNumRows, m_nNumColumns * 2);
		for(int i = 0; i < m_nNumRows; i++)
		{
			A.SetElement(i, m_nNumColumns + i, 1); // 单位阵
			A.SetElement(i, i, GetElement(i, i));  // 三对角矩阵
			if(i + 1 < m_nNumColumns)
			{
				A.SetElement(i, i + 1, GetElement(i, i + 1));
			}
			if(i - 1 >= 0)
			{
				A.SetElement(i, i - 1, GetElement(i, i - 1));
		...
		do
		{
			double m = A.GetElement(k, k);
			if(m == 0)
			{
				break;
			}
			// 第k+1行,第k列进行消元
			double s = A.GetElement(k + 1, k) / A.GetElement(k, k);
			for(int j = k; j < m_nNumColumns * 2; j++)
			{	
				A.SetElement(k + 1, j, A.GetElement(k + 1, j) - A.GetElement(k, j) * s);
			}
			k++;
		}while(k < m_nNumRows - 1);
		// 单位化
		for(int i = 0; i< m_nNumRows; i++)
		{	
			double s = A.GetElement(i, i);
			for(int j = i; j< m_nNumColumns * 2; j++)
				A.SetElement(i, j, A.GetElement(i, j) / s);
		}
		// 下三角,  0 - m_nNumRows - 2
		k = m_nNumRows - 1;
		do
		...
		Matrix matInv(m_nNumRows, m_nNumColumns);
		for(int i = 0; i < m_nNumRows; i++)
		{
			for(int j = 0; j < m_nNumColumns; j++)
			{
				matInv.SetElement(i, j, A.GetElement(i, m_nNumColumns + j));
			}
		}
		return matInv;
	}
	...
	{
		// 构造目标矩阵
		Matrix	Abs(m_nNumRows, m_nNumColumns); 
		for (int i = 0 ; i < m_nNumRows ; i++)
		{
			for (int j = 0 ; j < m_nNumColumns ; j++)
				Abs.SetElement(i, j, fabs(GetElement(i, j))) ;// abs--->fabs  20070731
		}
		return Abs;
	}
	...
	// 返回值：double型，最大矩阵元素
	//////////////////////////////////////////////////////////////////////
	double  Matrix::Max(int& nRow,int& nCol)
	{
		double bMax = GetElement(0, 0);
		nRow=0;nCol=0;
		for (int i = 0 ; i < m_nNumRows ; ++i)
		{
			for (int j = 0 ; j < m_nNumColumns ; ++j)
			{
				if(bMax < GetElement(i, j))//发现更大的元素
				{
					bMax = GetElement(i, j);
					nRow = i;
					nCol = j;
	...
	// 返回值：double型，最小矩阵元素
	//////////////////////////////////////////////////////////////////////
	double  Matrix::Min(int& nRow,int& nCol)
	{
		double bMin=GetElement(0, 0);
		nRow=0;nCol=0;
		for (int i = 0 ; i < m_nNumRows ; ++i)
		{
			for (int j = 0 ; j < m_nNumColumns ; ++j)
			{
				if(bMin>GetElement(i, j))//发现更小的元素
				{
					bMin=GetElement(i, j);
					nRow=i;
					nCol=j;
	...
	//
	// 参数：
	// 1. Matrix& mtxImag - 复矩阵的虚部矩阵，当前矩阵为复矩阵的实部
	//
	// 返回值：BOOL型，求逆是否成功
	//////////////////////////////////////////////////////////////////////
	BOOL Matrix::InvertGaussJordan(Matrix& mtxImag)
	{
	...
	// 返回值：BOOL型，求解是否成功
	//////////////////////////////////////////////////////////////////////
	BOOL Matrix::DetCholesky(double* dblDet)
	{ 
		int i,j,k,u,l;
		double d;
	...
	//
	// 参数：
	// 1. Matrix& mtxL - 返回分解后的L矩阵
	// 2. Matrix& mtxU - 返回分解后的U矩阵
	//
	// 返回值：BOOL型，求解是否成功
	//////////////////////////////////////////////////////////////////////
	BOOL Matrix::SplitLU(Matrix& mtxL, Matrix& mtxU)
	{ 
		int i,j,k,w,v,ll;
	...
	//
	// 参数：
	// 1. Matrix& mtxQ - 返回分解后的Q矩阵
	//
	// 返回值：BOOL型，求解是否成功
	//////////////////////////////////////////////////////////////////////
	BOOL Matrix::SplitQR(Matrix& mtxQ)
	{ 
		int i,j,k,l,nn,p,jj;
		double u,alpha,w,t;
	...
	//
	// 参数：
	// 1. Matrix& mtxU - 返回分解后的U矩阵
	// 2. Matrix& mtxV - 返回分解后的V矩阵
	// 3. double eps - 计算精度，默认值为0.000001
	//
	// 返回值：BOOL型，求解是否成功
	//////////////////////////////////////////////////////////////////////
	BOOL Matrix::SplitUV(Matrix& mtxU, Matrix& mtxV, double eps /*= 0.000001*/)
	{ 
	...
	// 内部函数，由SplitUV函数调用
	//////////////////////////////////////////////////////////////////////
	void Matrix::ppp(double a[], double e[], double s[], double v[], int m, int n)
	{ 
		int i,j,p,q;
		double d;
	...
	// 内部函数，由SplitUV函数调用
	//////////////////////////////////////////////////////////////////////
	void Matrix::sss(double fg[2], double cs[2])
	{ 
		double r,d;
	...
	//
	// 参数：
	// 1. Matrix& mtxAP - 返回原矩阵的广义逆矩阵
	// 2. Matrix& mtxU - 返回分解后的U矩阵
	// 3. Matrix& mtxV - 返回分解后的V矩阵
	// 4. double eps - 计算精度，默认值为0.000001
	//
	// 返回值：BOOL型，求解是否成功
	//////////////////////////////////////////////////////////////////////
	BOOL Matrix::GInvertUV(Matrix& mtxAP, Matrix& mtxU, Matrix& mtxV, double eps /*= 0.000001*/)
	{ 
		int i,j,k,l,t,p,q,f;
	...
	//
	// 参数：
	// 1. Matrix& mtxQ - 返回豪斯荷尔德变换的乘积矩阵Q
	// 2. Matrix& mtxT - 返回求得的对称三对角阵
	// 3. double dblB[] - 一维数组，长度为矩阵的阶数，返回对称三对角阵的主对角线元素
	// 4. double dblC[] - 一维数组，长度为矩阵的阶数，前n-1个元素返回对称三对角阵的次对角线元素
	//
	// 返回值：BOOL型，求解是否成功
	//////////////////////////////////////////////////////////////////////
	BOOL Matrix::MakeSymTri(Matrix& mtxQ, Matrix& mtxT, double dblB[], double dblC[])
	{ 
		int i,j,k,u;
		double h,f,g,h2;
	...
	// 3. Matrix& mtxQ - 如果传入单位矩阵，则返回实对称三对角阵的特征值向量矩阵；
	...
	// 返回值：BOOL型，求解是否成功
	//////////////////////////////////////////////////////////////////////
	BOOL Matrix::SymTriEigenv(double dblB[], double dblC[], Matrix& mtxQ, int nMaxIt /*= 60*/, double eps /*= 0.000001*/)
	{
		int i,j,k,m,it,u,v;
	...
	// 返回值：BOOL型，求解是否成功
	//////////////////////////////////////////////////////////////////////
	BOOL Matrix::HBergEigenv(double dblU[], double dblV[], int nMaxIt /*= 60*/, double eps /*= 0.000001*/)
	{ 
	...
	// 2. Matrix& mtxEigenVector - 返回时存放特征向量矩阵，其中第i列为与
	...
	// 返回值：BOOL型，求解是否成功
	//////////////////////////////////////////////////////////////////////
	BOOL Matrix::JacobiEigenv(double dblEigenValue[], Matrix& mtxEigenVector, int nMaxIt /*= 60*/, double eps /*= 0.000001*/)
	{ 
	...
	// 返回值：BOOL型，求解是否成功
	//////////////////////////////////////////////////////////////////////
	BOOL Matrix::JacobiEigenv2(double dblEigenValue[], Matrix& mtxEigenVector, double eps /*= 0.000001*/)
	{ 
		int i,j,p,q,u,w,t,s;
