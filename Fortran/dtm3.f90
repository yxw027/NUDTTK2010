...
!************************************************************************
  ...
  open(iu10,file='DTM_2020_F107_Kp.dat',status='old')  
  !
  read(iu10,530) titre
  read(iu10,540) npdtm
  !write(*,600) titre
  ...
  do i=1,npdtm
    ...
    ! 2021.12.10¡£Î¤´º²©
    ! write(*, *) ni,tt(i),dtt,h(i),dh,he(i),dhe,o(i),dox,az2(i),daz2,o2(i),do2,az(i),daz,t0(i),dt0,tp(i),dtp
  end do
  close(iu10);
!
20 continue
...
!
  ...
  pis12=pi/12.
!
  open(10,file='DTM_2020_F107_Kp.dat',status='old') 
  call lecdtm	!read in coefficients
! *****************  input test values  ********************************
  altl=300.		
  alth=800
  alat=0.
...
12 format('ro  :',4e12.5)   
    end subroutine moddtm
    
subroutine dtm2020(day, fl, fml, akp, altl, hl, alat, xlon, tz, tinf, rou, d, wmm)
  !***********************************************************************
! s. bruinsma (22/10/20) 
!
! compile [double precision] dtm2020_F107_Kp-subr.f90 Mod_DTM2020F107Kp_Benchmark.f90
!
  real, intent(in) :: day, altl, hl, alat, xlon
  real, intent(in), dimension(2) :: fl, fml
  real, intent(in), dimension(4) :: akp
  real, intent(out), dimension(6) :: d
  real, intent(out) :: rou, tz, tinf, wmm
...
!
  ...
  !write(*,*) 'this run:'  
! *********************************************************************
    !call dtm3(d_in,f_in,fml,akp,altl,hl,alat,xlon,tz_ll,tinf_ll,rou,par_ro,wmm)
    call dtm3(day, fl, fml, akp, altl, hl, alat, xlon, tz, tinf, rou, d, wmm)
    !call dtm3(day,fh,fmh,akp,altl,hl,alat,xlon,tz_lh,tinf_lh,ro_lh,par_ro,wmm)
    !call dtm3(day,fl,fml,akp,alth,hl,alat,xlon,tz_hl,tinf_hl,ro_hl,par_ro,wmm)
    !call dtm3(day,fh,fmh,akp,alth,hl,alat,xlon,tz_hh,tinf_hh,ro_hh,par_ro,wmm)
    !
