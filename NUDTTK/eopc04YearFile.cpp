#include "eopc04YearFile.hpp"
#include "TimeCoordConvert.hpp"

namespace NUDTTK
{
	Eopc04YearFile::Eopc04YearFile(void)
	{
	}
	...
	// 修改记录：
	// 备注： 
	bool Eopc04YearFile::isValidEpochLine(string strLine, Eopc04YearLine& eopc04line)
	{
		bool nFlag = true;
		Eopc04YearLine Line; // Eopc04Line 的构造函数要有必要的初始化, 以保证不出现错误的判断, 因为在读到最后一行时, strLine的有效字符很短, line无法通过strLine来赋值
	...
	// 备注： 
	bool Eopc04YearFile::open(string  strEopc04YearfileName)
	{
		FILE * pEopc04file = fopen(strEopc04YearfileName.c_str(),"r+t");
		if(pEopc04file == NULL) 
			return false;
		// 略过前 14 行
	...
	// 备注： 
	bool Eopc04YearFile::getPoleOffset(UTC t, double& x, double& y)
	{
		if(m_data.size() <= 1)
			return false;
	...
	//           3. 2018/05/06 由邵凯修改，多年数据
	// 备注： 
	bool Eopc04YearFile::getNutationCorrect(UTC t, double& psi, double& eps)
	{
		if(m_data.size() <= 1)
			return false;
		double mjd = TimeCoordConvert::DayTime2MJD(t);  // 获得修改的儒略日
	...
	// 修改记录：
	// 备注： 
	bool Eopc04YearFile::getUT1_UTC(UTC t,double& ut1_utc)
	{
		double ut1_utc_rate;
	...
	// 备注： 
	bool Eopc04YearFile::getUT1_UTC(UTC t, double& ut1_utc, double& ut1_utc_rate)
	{
