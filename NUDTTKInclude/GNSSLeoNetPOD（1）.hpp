...
namespace NUDTTK
{
	namespace SpaceborneGPSPod
	{
		...
		struct LEOPOD_StaDatum 
		{
			...
			bool      on_Cst_EmpAcc_R;
			bool      on_Cst_EmpAcc_T;
			bool      on_Cst_EmpAcc_N;
			...
			float         priorsigma_TangentialEmpAcc;  // T方向常值经验力约束
			float         priorsigma_NormalEmpAcc;  // N方向常值经验力约束
   ...
   //         Matrix                mat_a; // 记录参数估计结果: 动力学参数、模糊度、天线偏移、系统偏差
			...
			// 残差编辑
			void ocResEdit(float factor = 3.0f);
		...
		public:
			...
			bool mainNetPOD_MixedGNSS(GPST t0, GPST t1, double interval = 30.0); // 缩减动力学绝对定轨+相对定轨
			bool mainNetPOD_MixedGNSS_DD(GPST t0, GPST t1, double interval = 30.0); // 双差数据缩减动力学+运动学相对定轨
		...
		private:
			...
			void updateSta_NEQ(StaDatumMap::iterator it_Sta);         // + 更新测站法方程
